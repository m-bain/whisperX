...
def cli():
    ...
    parser.add_argument("--asr_backend", type=str, default="faster-whisper", choices=["faster-whisper", "qwen3"], help="ASR backend to use")
    parser.add_argument("--model", default="small", help="name of the Whisper model to use")
    parser.add_argument("--qwen_asr_model", default="Qwen/Qwen3-ASR-1.7B", help="Qwen ASR model id/path used when --asr_backend qwen3")
    parser.add_argument("--qwen_forced_aligner", default="Qwen/Qwen3-ForcedAligner-0.6B", help="Qwen forced aligner model id/path used when --asr_backend qwen3")
    parser.add_argument("--qwen_dtype", default="float16", choices=["float16", "float32", "bfloat16"], help="dtype used to load Qwen ASR/aligner models")
    parser.add_argument("--qwen_device_map", default=None, help="Optional Hugging Face device_map for Qwen models (e.g. auto). If unset, WhisperX --device is used.")
    ...
    parser.add_argument("--device", default="cuda" if torch.cuda.is_available() else "cpu", help="device type to use for PyTorch inference (e.g. cpu, cuda, mps)")
