...
from .vad import PyannoteVADPipeline, SileroVADPipeline, FasterWhisperSegment
...
class FasterWhisperModelAdapter:
    def __init__(self, *args, **kwargs):
        self.vad_pipeline = None
        self.device = kwargs.get("device", 'cpu')
        model_size_or_path = kwargs.pop("model_size_or_path", "small")
        self.model = WhisperModel(model_size_or_path, *args, **kwargs)
    
    @staticmethod
    def __audio_numpy_array_slice(audio: np.ndarray, start, end, sample_rate = 16000) -> np.ndarray:
        """Slice a numpy array of audio samples.

        Sample rate shall always be 16KHz for Silero VAD.
        """
            ...
            output["segments"].append(
                    {
                        ...
                        "language": language,
                        "text": segment.text,
                        "seg-text": [x.word for x in segment.words],
                        "seg-start": [x.start for x in segment.words],
                        "seg-end": [x.end for x in segment.words],
                    }
            )
        output["language"] = audio_info.language
        return output
    ...
    def transcribe(
        ...
        result_transcription, audio_info = self.model.transcribe(audio = audio, temperature = temperature,
                                                                 ...
                                                                 condition_on_previous_text,
                                                                 **decode_options)
        return self.format_output(result_transcription, audio_info, verbose = verbose)
    ...
    def __transcribe_with_vad(self, *args, **kwargs) -> Tuple[Iterable[FasterWhisperSegment], AudioInfo]:
        ...
        merged_segments = [FasterWhisperSegment(start = segment['start'],
                           ...
                           for segment in merged_segments_dicts]
        
        # cut audio into segments to feed into Faster-Whisper
        audio_segments = self.__audio_segment_generator(audio_arr, merged_segments)
        
        vad_transcript_segments = []
        
        kwargs['word_timestamps'] = True  # Need this for detailed segment info
        ...
        for (transcribe_segments, audio_info), segment in tqdm(zip(vad_transcript_segments, merged_segments)):
            if verbose:
                print(
                        f"~~ Transcribing VAD chunk: ({format_timestamp(segment.start)} --> "
        ...
        for segment_info, segments in zip(merged_segments, segment_transcription):
            for segment in segments:
                result_transcription.append(
                        FasterWhisperSegment(start = segment.start + segment_info.start,
                                             end = segment.end + segment_info.start,
                                             text = segment.text, speaker = None,
                                             words = segment.words))
        
        avg_language_probability = sum([audio_info.language_probability for audio_info in audio_infos]) / \
                                   len(audio_infos)
        
        total_duration = sum([segment.end - segment.start for segment in result_transcription])
        audio_info_merged = AudioInfo(language = audio_infos[0].language,
                                      language_probability = avg_language_probability, duration = total_duration)
        
        return result_transcription, audio_info_merged
