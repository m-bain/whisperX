...
from .utils import exact_div, format_timestamp, optional_int, optional_float, str2bool, write_txt, write_vtt, \
    write_srt, \
    write_ass, write_tsv
from .vad import PyannoteVADPipeline, SileroVADPipeline, Binarize
import pandas as pd
...
def cli():
    ...
    parser.add_argument("--model", default = "small", choices = available_models(),
                        help = "name of the Whisper model to use")
    parser.add_argument("--model_dir", type = str, default = None,
                        help = "the path to save model files; uses ~/.cache/whisper by default")
    parser.add_argument("--device", default = "cuda" if torch.cuda.is_available() else "cpu",
                        help = "device to use for PyTorch inference")
    parser.add_argument("--faster_whisper", action = "store_true",
                        help = "Whether to use Faster-Whisper with CTranslate2 than original Whisper model from OpenAI")
    ...
    parser.add_argument("--align_from_prev", default = True, type = bool,
                        help = "Whether to clip the alignment start time of current segment to the end time of the "
                               "last aligned word of the previous segment")
    parser.add_argument("--interpolate_method", default = "nearest", choices = ["nearest", "linear", "ignore"],
                        help = "For word .srt, method to assign timestamps to non-aligned words, or merge them into "
                               "neighbouring.")
    # vad params
    parser.add_argument("--vad_filter", action = "store_true",
                        help = "Whether to first perform VAD filtering to target only transcribe within VAD. Produces "
                               "more accurate alignment + timestamp, requires more GPU memory & compute.")
    parser.add_argument("--vad_model", default = "silero", choices = ['silero', 'pyannote'],
                        help = "name of the VAD model to use")
    ...
    parser.add_argument("--task", type = str, default = "transcribe", choices = ["transcribe", "translate"],
                        help = "whether to perform X->X speech recognition ('transcribe') or X->English translation ("
                               "'translate')")
    ...
    parser.add_argument("--patience", type = float, default = None,
                        help = "optional patience value to use in beam decoding, "
                               "as in https://arxiv.org/abs/2204.05424, the default (1.0) is equivalent to "
                               "conventional beam search")
    parser.add_argument("--length_penalty", type = float, default = None,
                        help = "optional token length penalty coefficient (alpha) as in "
    ...
    parser.add_argument("--suppress_tokens", type = str, default = "-1",
                        help = "comma-separated list of token ids to suppress during sampling; '-1' will suppress "
                               "most special characters except common punctuations")
    ...
    parser.add_argument("--no_speech_threshold", type = optional_float, default = 0.6,
                        help = "if the probability of the <|nospeech|> token is higher than this value AND the "
                               "decoding has failed due to `logprob_threshold`, consider the segment as silence")
    parser.add_argument("--threads", type = optional_int, default = 0,
                        help = "number of threads used by torch for CPU inference; supercedes "
    ...
    vad_pipeline = None
    if vad_filter:
        if vad_model_name == 'pyannote':
            assert hf_token is not None, "pyannote VAD model requires Hugging Face token"
            vad_pipeline = PyannoteVADPipeline(device = device,
                                               hf_token = hf_token)
        else:
            vad_pipeline = SileroVADPipeline(device = device)
    ...
    if faster_whisper:
        from model import FasterWhisperModelAdapter
        if model_dir:
    ...
    align_language = args["language"] if args[
                                             "language"] is not None else "en"  # default to loading english if not
    # specified
    ...
    for audio_path in args.pop("audio"):
        if vad_filter:
            if parallel_bs > 1:
                ...
                if faster_whisper:
                    result = model.transcribe_with_vad(audio = audio_path, vad_pipeline = vad_pipeline,
                                                       temperature = temperature, **args)
                else:
                    result = transcribe_with_vad(model = model, audio = audio_path, vad_pipeline = vad_pipeline,
                                                 temperature = temperature, **args)
        else:
            print("Performing transcription...")
            if faster_whisper:
                result = model.transcribe(audio = audio_path, temperature = temperature, **args)
            else:
                result = transcribe(model = model, audio = audio_path, temperature = temperature, **args)
